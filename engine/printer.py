...
class Printer(object):
    'Printer object'
    @staticmethod
    def print_spaceex_homo_odes(A, C, init_xmin_vec, init_xmax_vec, stoptime, step, file_name):
        ...
        cfg_file = print_spaceex_cfg_file_autonomous_ode(init_xmin_vec, init_xmax_vec, ymin_vec, ymax_vec, stoptime, step, cfg_file_name)

        return xml_file, cfg_file

    @staticmethod
    def print_spaceex_non_homo_odes(A, B, C, init_xmin_vec, init_xmax_vec, umin_vec, umax_vec, stoptime, step, file_name):
        'Print homogenous ODE dot{x} = Ax + Bu, y = Cx to SpaceEx format'
        pass

    @staticmethod
    def print_flow_homo_odes(A, init_xmin_vec, init_xmax_vec, stoptime, step, file_name):
        ...
        assert isinstance(init_xmin_vec, list) and isinstance(init_xmax_vec, list), 'error: xmin and xmax should be a list'
        assert isinstance(file_name, str), 'error: file name should be a string'
        assert A.shape[0] == len(init_xmin_vec) == len(init_xmax_vec), 'error: inconsistent dimensions between system matrix and initial vectors'
        ...
        for i in xrange(0, n):
            flow_model.write('  x{} in [{}, {}] \n'.format(i, init_xmin_vec[i], init_xmax_vec[i]))
    ...
    @staticmethod
    def print_flow_non_homo_odes(A, B, C, init_min_vec, init_max_vec, umin_vec, umax_vec, stoptime, step, file_name):
